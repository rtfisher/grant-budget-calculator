...
def main():
    ...
    number_grads = int(input("Enter number of graduate students [1]: ") or "1")
    if number_grads < 0:
        print("Error: number of graduate students cannot be negative.")
        sys.exit(1)

    # All graduate students are assumed to have the same stipend, fees, and insurance.
    if number_grads > 1:
        print(f"Note: all {number_grads} graduate students are assumed to have the same salary.")
    ...
    grad_salary_per = int(input(f"Enter graduate student annual stipend [{d_grad}]: ") or d_grad)
    grad_fees_per = float(input(f"Enter graduate student tuition + fees [{d_fees}]: ") or d_fees)
    grad_ins_per = float(input(f"Enter graduate student health insurance [{d_ins}]: ") or d_ins)
    undergrad_salary = int(input(f"Enter undergraduate student salary [{d_ugrad}]: ") or d_ugrad)

    grad_salary = number_grads * grad_salary_per
    grad_fees = number_grads * grad_fees_per
    grad_ins = number_grads * grad_ins_per
